/*
  # Purpose and Gamification System
  ...
  2. Table Modifications
    ...
    - Add `xp_awarded` to `micro_actions` and `outcomes` tables
    - Add `current_xp` and `level` to `identity_areas` table
...
-- Create purposes table (referencing auth.users explicitly)
...
-- Add foreign key constraint to purposes table (separate step for better error handling)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'purposes_user_id_fkey'
  ) THEN
    ALTER TABLE purposes ADD CONSTRAINT purposes_user_id_fkey 
    FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
  END IF;
EXCEPTION
  WHEN OTHERS THEN
    -- If auth.users doesn't exist, try public.users
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.table_constraints
      WHERE constraint_name = 'purposes_user_id_fkey'
    ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE outcomes ADD COLUMN purpose_id uuid;
  END IF;
END $$;

-- Add foreign key constraint for purpose_id in outcomes
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'outcomes_purpose_id_fkey'
  ) THEN
    ALTER TABLE outcomes ADD CONSTRAINT outcomes_purpose_id_fkey 
...
BEGIN
  ...
  new_xp := COALESCE(current_area_xp, 0) + xp_amount;
...
BEGIN
  ...
  IF action_area_id IS NOT NULL AND action_xp IS NOT NULL THEN
...
BEGIN
  ...
  IF NEW.status = 'completed' AND (OLD.status IS NULL OR OLD.status != 'completed') THEN
    ...
    -- Award XP if there's an identity area
    IF outcome_area_id IS NOT NULL AND outcome_xp IS NOT NULL THEN
...
-- Function to get user streak (if not already exists)
